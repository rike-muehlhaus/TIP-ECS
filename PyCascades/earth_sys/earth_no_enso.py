...
from core.tipping_element import cusp, cusp_new
...
class earth_system():
    ...
    def dynamic_earth_network(self, gmt_function, strength, kk0, kk1):
        gis = cusp_new(a=-1 / self._gis_time, b=1 / self._gis_time, c= lambda t : (1 / self._gis_time) * global_functions.CUSPc(0., self._limits_gis, gmt_function(t)))
        thc = cusp_new(a=-1 / self._thc_time, b=1 / self._thc_time, c=lambda t :(1 / self._thc_time) * global_functions.CUSPc(0., self._limits_thc, gmt_function(t)))
        wais = cusp_new(a=-1 / self._wais_time, b=1 / self._wais_time, c=lambda t :(1 / self._wais_time) * global_functions.CUSPc(0., self._limits_wais, gmt_function(t)))
        amaz = cusp_new(a=-1 / self._amaz_time, b=1 / self._amaz_time, c=lambda t :(1 / self._amaz_time) * global_functions.CUSPc(0., self._limits_amaz, gmt_function(t)))
        ...
        net.add_coupling(1, 3, linear_coupling(strength=(1 / self._amaz_time) * strength * self._pf_thc_to_amaz * kk1, x_0=-1))
