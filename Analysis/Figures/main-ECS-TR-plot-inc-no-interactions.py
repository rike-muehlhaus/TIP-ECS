...
for i, s in enumerate(reversed(scenarios)):
    ...
    ax.plot(x, y, color=color, zorder=4,  lw=1, label=f"{s} ppm")
    ax.plot(no_interactions_x, no_interactions_y, lw=0.5, alpha = 0.4, color=color, zorder=3)
...
ax.text(4.55, 0.4, "50% tipping risk", color="black", ha="left")
...
style_handles = [
    Line2D([0], [0], color=scenario_colors[::-1][6], lw=1, label="Interactions"),
    Line2D([0], [0],  color=scenario_colors[::-1][6], lw=0.5, alpha = 0.4, label="No interactions")
]
...
plt.subplots_adjust(right=0.68, left=0.13, top=0.92, bottom=0.13)
#plt.tight_layout()
#plt.show()
